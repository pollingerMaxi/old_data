//
// AdCase.js LIGHT JavaScript Library v2.1.16. 27/Feb/2018
...
ads.slotRendered = function(event) {
  if(!ads.debugChecked) { ads.checkDebug(); }
...
ads.matchAds = function() {
  for (var i3 in ads.adTexts) {
    var text = ads.adTexts[i3].text;
    if(!text) continue;
    var j2 = 0;
    var j3 = "";
    try {
      var sp = text.split(/\r\n|\r|\n/);
      for (var j3=0; j3<sp.length; j3++) {
        var sp2 = sp[j3];
        j2 = sp2.split("=");
        if (j2[1] && j2[1] > 10000) {
          ads.searchSlots(j2[1], ads.adTexts[i3].slotWindow);
        }
      }
    } catch(e) { console.log(e,text,i3,j2,j3,sp,sp2);debugger;}
  }
}
...
ads.formats.push = function(t) {
  ...
  t.msg = function(p) {
    ...
    if(p.action == "collapse") {
      var height = t.get("collapsedHeight") || ads.styles.push.collapsedHeight || 90;
      t.parentSlot.style.height = height + "px";  
    } else if(p.action == "expand") {
      var height = t.get("expandedHeight") || ads.styles.push.expandedHeight || 250;
      t.parentSlot.style.height = height + "px"; 
    }
  }
